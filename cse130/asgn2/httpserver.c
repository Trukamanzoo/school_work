...
#include <string.h>
#include <errno.h>
#include "httpserver.h" // Assuming this is the header file containing the helper functions and structs
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (strcmp(method, "GET") == 0) {
            ...
            char file_path[65];
            sprintf(file_path, ".%s", uri); // Assuming the server is running in the directory with the files
            ...
            if (file == NULL) {
                ...
                // Read the file contents and send a 200 OK response with the file contents as the message body
                char file_contents[2048]; // Adjust size as needed
                size_t len = fread(file_contents, 1, sizeof(file_contents) - 1, file);
                file_contents[len] = '\0';
            ...
            if (file == NULL) {
                ...
                // Write the message body to the file and send a 200 OK or 201 Created response
